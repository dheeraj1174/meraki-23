...
export const updateEventValidator = async (values: Partial<ParsedEvent>) => {
	const errors: Partial<ParsedEvent> = {};
	...
	if (values.registrationsStart && !values.registrationsStart.trim()) {
		errors.registrationsStart = "Please provide a valid date";
	} else if (
		values.registrationsStart &&
		isNaN(Date.parse(values.registrationsStart))
	) {
		errors.registrationsStart = "Please provide a valid date";
	}

	if (values.registrationsEnd && !values.registrationsEnd.trim()) {
		errors.registrationsEnd = "Please provide a valid date";
	} else if (
		values.registrationsEnd &&
		isNaN(Date.parse(values.registrationsEnd))
	) {
		errors.registrationsEnd = "Please provide a valid date";
	}

	if (values.eventStart && !values.eventStart.trim()) {
		errors.eventStart = "Please provide a valid date";
	} else if (values.eventStart && isNaN(Date.parse(values.eventStart))) {
		errors.eventStart = "Please provide a valid date";
	}

	if (values.eventEnd && !values.eventEnd.trim()) {
		errors.eventEnd = "Please provide a valid date";
	} else if (values.eventEnd && isNaN(Date.parse(values.eventEnd))) {
		errors.eventEnd = "Please provide a valid date";
	}
	...
	if (values.image && !values.image.trim()) {
		errors.image = "Please provide link to a poster image";
	} else if (values.image && !regex.avatar.test(values.image))
		errors.image = "Please provide a valid image URL";

	if (values.teamSize && values.teamSize === "") {
		errors.teamSize =
			"Please enter maximum number of members in a team (1 for solo participation)";
	} else if (values.teamSize && isNaN(Number(values.teamSize))) {
		errors.teamSize = "Please enter a valid number";
	}
